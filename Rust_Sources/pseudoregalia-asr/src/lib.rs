...
async fn main() {
    ...
    loop {
        ...
        if let Some(process) = PROCESS_NAMES.iter().find_map(|name| {
                process_name = name;
                Process::attach(name)
            })
        {
            process
              .until_closes(async {
                  ...
                  {
                      let size =
                        retry(|| process.get_module_size(process_name)).await;
                      match size {
                          2125824 => {
                  ...
                  if version.is_some() {
                      ...
                      if let Some(module) = Module::attach(
                          &process,
                          Version::V5_1,
                          process.get_module_address(process_name).unwrap(),
                      )
                      {
                          let g_world = retry(|| module.get_g_world_uobject(&process)).await;
                          ...
                          loop {
                              ...
                              if let Ok(flag) = g_world.get_fname::<40>(&process, &module) {
                                  set_variable("G World Name", flag.validate_utf8().unwrap())
                              }
                              if just_started {
                                  just_started = false;
